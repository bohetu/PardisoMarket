...
bool compareMatrixEntry(MatrixEntry const &a, MatrixEntry const &b)
{
    if (a.row == b.row)
        return a.col < b.col;
    return a.row < b.row;
...
void say(const char *filename)
{
    printf("%s\n", filename);
}
int mm_read_csr3_mtx(const char *filename, MKL_INT N_)
{
    FILE *f;
    MM_typecode matcode;
    int M, N, nz;
...
//    *ja_ = ja;
    ...
    std::sort(matrix.begin(), matrix.end(), compareMatrixEntry);
